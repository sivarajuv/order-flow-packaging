...
import { getUsers, createUser, updateUser, setUserActive, deleteUser } from '../api/client'
import { Badge, Modal, Spinner, RowActions } from '../components/UI'
...
export default function UserManagement() {
  ...
  const [modal,      setModal]      = useState(null)   // null | 'create' | 'edit' | 'pw' | 'changepw'
  ...
  const saveEdit = async () => {
    ...
    setSaving(true)
    try {
      const payload = { fullName: form.fullName.trim(), role: form.role }
      if (form.password) payload.password = form.password
      const u = await updateUser(selected.id, payload)
  ...
  const saveChangePw = async () => {
    ...
    try {
      ...
      }).then(async r => { if (!r.ok) { const d = await r.json(); throw new Error(d.message) } })
      setModal(null)
    ...
    <div className="page">
      <div className="page-header">
        ...
        <div className="page-actions">
          <button className="btn" onClick={openChangePw}>
            🔑 Change my password
          </button>
        ...
        {ROLE_OPTS.map(r => {
          ...
          return (
            <div key={r.value} style={{
              background: '#fff', border: `1px solid ${c.bg}`, borderRadius: 10,
                ...
                <tr key={u.id} style={{ opacity: u.active ? 1 : 0.55 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                      ...
                      <div>
                        <div style={{ fontWeight: 600, fontSize: 12 }}>
                          {u.fullName}
                          {u.id === me?.id && (
                            <span style={{ marginLeft: 6, fontSize: 9, fontWeight: 700, color: '#6D28D9', background: '#F5F3FF', padding: '1px 5px', borderRadius: 10 }}>
                              YOU
                            </span>
                          )}
                        </div>
                  ...
                  <td>
                    <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                      <button className="btn btn-sm" onClick={() => openEdit(u)} title="Edit role / name">
                        Edit
                      </button>
                      <button className="btn btn-sm" onClick={() => openResetPw(u)} title="Reset password">
                        🔑 Reset
                      </button>
                      {u.id !== me?.id && (
                        <>
                          <button
                            className={`btn btn-sm ${u.active ? '' : 'btn-success'}`}
        ...
        <Modal title="Create new user" onClose={() => setModal(null)}>
          <div className="form-grid">
            <div className="field">
              ...
              <input value={form.username} onChange={e => set('username', e.target.value)} placeholder="e.g. john.doe" autoFocus />
            </div>
            <div className="field">
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {ROLE_OPTS.map(r => (
                  <label key={r.value} style={{
                    flex: 1, cursor: 'pointer', border: `2px solid ${form.role === r.value ? ROLE_COLORS[r.value].color : 'var(--border)'}`,
                    borderRadius: 8, padding: '8px 10px', background: form.role === r.value ? ROLE_COLORS[r.value].bg : '#fff',
                    transition: 'all .1s',
                  }}>
                    <input type="radio" name="role" value={r.value} checked={form.role === r.value} onChange={e => set('role', e.target.value)} style={{ display: 'none' }} />
                    <div style={{ fontWeight: 600, fontSize: 11, color: form.role === r.value ? ROLE_COLORS[r.value].color : 'var(--ink)' }}>{r.label}</div>
                    <div style={{ fontSize: 10, color: 'var(--ink2)', marginTop: 2 }}>{r.desc}</div>
                  </label>
                ))}
              </div>
            </div>
            ...
            <div className="field">
              ...
              <input type="password" value={form.confirmPw} onChange={e => set('confirmPw', e.target.value)} placeholder="Confirm password" />
            </div>
          </div>
          <div className="modal-footer">
            <button className="btn" onClick={() => setModal(null)}>Cancel</button>
            <button className="btn btn-primary" onClick={saveCreate} disabled={saving}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {ROLE_OPTS.map(r => (
                  <label key={r.value} style={{
                    ...
                    <input type="radio" name="edit-role" value={r.value} checked={form.role === r.value} onChange={e => set('role', e.target.value)} style={{ display: 'none' }} />
                    <div style={{ fontWeight: 600, fontSize: 11, color: form.role === r.value ? ROLE_COLORS[r.value].color : 'var(--ink)' }}>{r.label}</div>
                    <div style={{ fontSize: 10, color: 'var(--ink2)', marginTop: 2 }}>{r.desc}</div>
                  </label>
                ))}
      ...
      {/* ── Reset password modal (admin resets for other user) ── */}
      {modal === 'pw' && selected && (
        <Modal title={`Reset password — ${selected.username}`} onClose={() => setModal(null)}>
          <p style={{ fontSize: 12, color: 'var(--ink2)', marginBottom: 14 }}>
            Set a new password for <strong>{selected.fullName}</strong>. They should change it after next login.
          </p>
          <div className="form-grid">
            <div className="field">
            ...
            <div className="field">
              ...
              <input type="password" value={form.confirmPw} onChange={e => set('confirmPw', e.target.value)} />
            </div>
          </div>
          <div className="modal-footer">
            <button className="btn" onClick={() => setModal(null)}>Cancel</button>
            <button className="btn btn-primary" onClick={saveResetPw} disabled={saving}>
        ...
        <Modal title="Change my password" onClose={() => setModal(null)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 13 }}>
            <div className="field">
              ...
              <input type="password" value={pwForm.currentPassword} onChange={e => setPw('currentPassword', e.target.value)} autoFocus />
            </div>
            <div className="field">
              <label>New password * (min 6 chars)</label>
              <input type="password" value={pwForm.newPassword} onChange={e => setPw('newPassword', e.target.value)} />
            </div>
            <div className="field">
              <label>Confirm new password *</label>
              <input type="password" value={pwForm.confirmPw} onChange={e => setPw('confirmPw', e.target.value)} />
            </div>
          </div>
          <div className="modal-footer">
            <button className="btn" onClick={() => setModal(null)}>Cancel</button>
            <button className="btn btn-primary" onClick={saveChangePw} disabled={saving}>
