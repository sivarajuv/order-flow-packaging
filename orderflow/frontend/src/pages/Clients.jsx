...
import {
  ...
  getClientProducts, addClientProduct, updateClientProduct, deleteClientProduct, uploadClientDesign,
  getProducts,
} from '../api/client'
...
export function ClientsList() {
  ...
  const blank = () => ({
    ...
    billingAddress: '', shippingAddress: '', placeOfSupply: '', creditLimit: '', paymentTerms: 'Net 30',
  ...
  const GST_OPTS = [{ value: 0, label: '0% exempt' }, { value: 5, label: '5%' }, { value: 18, label: '18%' }, { value: 28, label: '28%' }]
  ...
  const filteredClients = clients.filter(c => {
    ...
    const matchesQuery = !text || [c.code, c.name, c.areaCode, c.salesperson, c.phone]
    ...
    <div className="page">
      ...
      <div className="card" style={{ marginBottom: 14 }}>
        <div className="form-grid">
          <div className="field">
            <label>Search</label>
            <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Code, name, area, salesperson, phone" />
          </div>
          <div className="field">
            <label>Status</label>
                ...
                return (
                  ...
                  >
                    ...
                    <td onClick={e => e.stopPropagation()}>
                      <RowActions editing={isEditing} saving={saving} onEdit={() => startEdit(c)} onSave={saveRow} onCancel={cancelEdit} />
                    </td>
                  </tr>
                )
              })}
    ...
    <div className="page">
      ...
      <div className="page-header">
        ...
        <div className="page-actions">
          ...
          {!editClient && (
            <button className="btn" onClick={() => nav('/orders', { state: { clientId: client.id } })}>
              + Sales order
            </button>
          )}
          {!editClient && (
            <button className="btn btn-primary" onClick={() => nav('/payments', { state: { clientId: client.id } })}>
              + Payment
            </button>
          )}
          {!editClient
      ...
      <div className="detail-grid">
        <div className="card">
          ...
          <DetailRow label="Shipping address" value={
            editClient
              ? <input className="cell-input" style={{ width: 210 }} value={clientForm.shippingAddress || ''} onChange={e => setCF('shippingAddress', e.target.value)} />
              : <span className="text-muted">{client.shippingAddress || '—'}</span>
          } />
          <DetailRow label="Place of supply" value={
            editClient
              ? <input className="cell-input" style={{ width: 210 }} value={clientForm.placeOfSupply || ''} onChange={e => setCF('placeOfSupply', e.target.value)} placeholder="e.g. Maharashtra or 27-Maharashtra" />
              : <span className="text-muted">{client.placeOfSupply || '—'}</span>
          } />
        </div>
      ...
      <div className="card" style={{ marginBottom: 14 }}>
        <div className="card-header">
          <div className="card-title">Client design</div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
          <div>
            <div style={{ fontSize: 12, color: 'var(--ink2)', marginBottom: 6 }}>
              Upload the approved design for this client. The system can use that client-specific design in further processing.
            </div>
            <div className="text-muted text-small">
              Current file: <strong>{client.designFileName || 'No design uploaded'}</strong>
            </div>
            {client.designUrl && (
              <div style={{ marginTop: 8 }}>
                <a href={client.designUrl} target="_blank" rel="noreferrer">View uploaded design</a>
              </div>
            )}
          </div>
          <label className="btn btn-primary" style={{ cursor: uploadingDesign ? 'wait' : 'pointer', opacity: uploadingDesign ? 0.7 : 1 }}>
            ...
            <input
              ...
              onChange={async e => {
                ...
                try {
                  const updated = await uploadClientDesign(id, file)
                  setClient(updated)
                  setClientForm({ ...updated })
                  toast.success('Client design uploaded')
                } finally {
